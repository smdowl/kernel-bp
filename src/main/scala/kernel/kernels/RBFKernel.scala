...
class RBFKernel extends Kernel {
   override def apply(p1: DenseMatrix[Double], p2: DenseMatrix[Double], deg: Double = 1.0): DenseMatrix[Double] = {
     ...
     val G: DenseMatrix[Double] = sum(p1 :* p1, Axis._1).asInstanceOf[DenseVector[Double]].toDenseMatrix.t
     val H: DenseMatrix[Double] = sum(p2 :* p2, Axis._1).asInstanceOf[DenseVector[Double]].toDenseMatrix.t
